...
class MinHeap2 {
	...
	public MinHeap2(List<Integer> list) {
		heap = buildHeap(list);
	}
	...
	private void siftDown(int currentIndex, int lastIndex, List<Integer> heap) {
		...
		while (leftChildIndex <= lastIndex) {
			...
			if (rightChildIndex <= lastIndex && heap.get(rightChildIndex) != null) {
			...
			if (heap.get(currentIndex) < heap.get(smallestChildIndex)) {
				...
				swap(currentIndex, smallestChildIndex, heap);
			}
	...
	public static void main(String args[]) {
		...
		MinHeap minHeap2 = new MinHeap2(list);
