 ...
 *
 * Description:
 * Given elevation map with heights compute how much water can be trapped after
 * raining
 *
 * Solution:
 ...
 * 1. Runtime: O(n) time: 3 * O(n) => O(n): 3 for loops for left/right max and
 * get water trapped at each index
...
class TrappingRainWater {
    ...
    public int computeWaterTrapped(int[] heights) {
        ...
        // Start from j = size - 2, because there is no j + 1 at the last index (size -
        // 1)
         ...
         * 
         * Iterate to < (size - 1) because cannot trap water at the end (no "wall" at
         * last index).
         * Same with 0th index. Start trapping water at index 2 and size - 2 (so iterate
         * to < size - 1).
         */
