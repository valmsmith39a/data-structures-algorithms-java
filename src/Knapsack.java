...
class Knapsack {
	/**
	 * Prompt:
	 * Given an array of arrays where each sub-array holds 2 integer values and
	 * represents items.
	 * First integer: item's value
	 * Second integer: item's weight
	 * Also given the maximum capacity of the knapsack
	 * 
	 * Find the number of items that you can fit into your knapsack that would
	 * maximize the
	 ...
	 * b. Items as rows
	 * c. At each max capacity, with x number of items, what is the max value of
	 * items?
	 * 2. For each value[i][j], ask, what is the max value of items I can have at
	 * this max capacity?
		...
		for (int i = 1; i < items.length + 1; i++) {
			...
			for (int c = 0; c < capacity + 1; c++) {
				if (currentWeight > c) {
					...
					// max of { knapsackValues[i - 1][c - currentWeight] + currentValue,
	...
	static List<List<Integer>> getKnapsackItems(int[][] knapsackValues, int[][] items, int weight) {
		...
		System.out.println("knapsack values len " + knapsackValues[0].length);
